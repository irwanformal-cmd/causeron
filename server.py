#!/usr/bin/env python3
"""Causeron — self-contained swarm-intelligence engine server.
...
runs anywhere Python 3.9+ is available:
    ...
    CAUSERON_PORT=8080 python3 server.py
...
log = logging.getLogger("causeron")


def _setup_logging() -> None:
    if log.handlers:  # idempotent (tests may re-import)
        return
    log.setLevel((os.environ.get("CAUSERON_LOG_LEVEL", "INFO")).upper())
...
_RATE_LIMIT = int(os.environ.get("CAUSERON_RATE_LIMIT", "600"))
_RATE_WINDOW = 60.0
...
_HEAVY_RATE_LIMIT = int(os.environ.get("CAUSERON_RATE_LIMIT_HEAVY", "30"))
...
_AUTH_TOKEN = os.environ.get("CAUSERON_TOKEN", "")
...
def handle_report(body: dict) -> dict:
    ...
    try:
        ...
        logging.getLogger("causeron").exception("analysis layer failed")
    with _lock:
        p["report"] = rep
        _save_project(p)
    return rep
...
def handle_compare(body: dict, on_progress=None) -> dict:
    ...
    # provider tolerates concurrent requests (tune CAUSERON_LLM_WORKERS if not).
...
def _to_markdown(p: dict) -> str:
    lines = [f"# {p['name']}", "", f"*Generated by Causeron — {p['created']}*", ""]
...
<title>{_esc(p['name'])} · Causeron report</title>
<style>
...
<h1>◈ {_esc(p['name'])}</h1>
<p class="meta">Generated by Causeron · {_esc(p.get('created'))} · {web.get('n_nodes', '?')} events, {web.get('n_edges', '?')} causal links</p>
...
<footer>Causeron · causal prediction engine — static export, no live data.</footer>
</body></html>"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _route(self):
        ...
        # bearer token guard (when CAUSERON_TOKEN is set; /api/health stays open)
...
def main():
    ...
    port = int(os.environ.get("CAUSERON_PORT", "8000"))
    host = os.environ.get("CAUSERON_HOST", "127.0.0.1")
    ...
    if host not in ("127.0.0.1", "localhost", "::1") and not _AUTH_TOKEN:
        warn = ("WARNING: binding to a non-localhost address WITHOUT CAUSERON_TOKEN — "
                "the entire API (projects, LLM settings, paid endpoints) is open "
                "to the network. Set CAUSERON_TOKEN or bind to 127.0.0.1.")
        log.warning(warn)
        print("  ⚠️  " + warn + "\n")
    try:
        httpd.serve_forever()
